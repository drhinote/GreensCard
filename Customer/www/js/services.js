...
var unwatchBucket
    ...
    return {
        ...
        waitForPayment: function(slot, amount, cb) {
                ...
                bucket.$save();
                bucket.message = "Connecting to customer";
                child.modal = $ionicModal.fromTemplate('<ion-modal-view><br/><br/><h3> Payment Status</h3><br/><br/><p> {{ bucket.message }}</p><br/><br/><div class="button button-clear icon-left ion-android-checkmark-circle" ng-show="bucket.completed">Payment Complete</div><div class="button button-clear icon-left ion-alert-circled" ng-show="bucket.declined">Payment Declined</div><div class="button button-clear icon-left ion-alert-circled" ng-show="bucket.failed">Payment Error</div><button ng-click="close()" class="button button-balanced button-block icon-left ion-close">Go Back</button><div id="firebaseui-auth-container"></div></ion-modal-view>', {
                    scope: child
                  });
